...
{
    [[TBExceptionManager sharedInstance] performSelectorOnMainThread:@selector(handleException:) withObject:exception waitUntilDone:YES];
}
...
- (void)uploadDeviceMessageComplete:(void(^)(id result, NSError *error))complete;
{
    ...
    //NSLog(@"%@", param);
    [[TBBusinessClient sharedInstance] getDeviceMessageWithParam:param complete:^(id result, NSError *error) {
...
{
    TBPerformanceMonitor *monitor = [TBPerformanceMonitor sharedInstance];
    if (self.appCode) {
        monitor.appCode = self.appCode;
    }
    if (self.macAddress) {
        monitor.macAddress = self.macAddress;
    }
    [monitor start];
}
